...
export default function Kiosk({ onCustomerChange, onLoginStateChange }) {
  ...
  // Listen for logout requests from top nav
  useEffect(() => {
    const handleExtLogout = () => {
      setCustomer(null);
    ...
    window.addEventListener('kiosk-logout', handleExtLogout);
    return () => window.removeEventListener('kiosk-logout', handleExtLogout);
  }, [onCustomerChange]);

  // Report login state to parent (App.js) so it can hide the nav-welcome area
  useEffect(() => {
    if (onLoginStateChange) {
      onLoginStateChange(showCustomerLogin);
    }
  }, [showCustomerLogin, onLoginStateChange]);
      ...
      <div className="kiosk-content-wrapper">
        ...
        {view === 'ADDONS' && currentItemIndex !== null && (
          <button className="back-arrow" onClick={() => {
            setOrderItems(prev => prev.filter((item, idx) => !(idx === currentItemIndex && item.isDraft)));
            setCurrentItemIndex(null);
            setView('ITEMS');
          }}>
            ← Back to Menu
            ...
            <div className="addons-view">
              ...
              <AddonsPanel
                ...
                onDone={() => {
                  setOrderItems(prev => {
                    const copy = [...prev];
                    if (copy[currentItemIndex]) {
                      delete copy[currentItemIndex].isDraft;
                    }
                    return copy;
                  });
                  setCurrentItemIndex(null);
        ...
        <aside className="order-sidebar glass-card">
          <div className="order-sidebar-header">
            ...
            {customer && (
              <span className="order-stars" title={`${customer.points ?? 0} stars`}>
                <span aria-hidden="true">&#9733;</span> {customer.points ?? 0}
              </span>
            )}
          </div>
          ...
          <div className="order-items-list">
            ...
            {orderItems.map((item, i) => {
              const isEditing = view === 'ADDONS' && i === currentItemIndex;
              if (item.isDraft && !isEditing) return null;
      ...
      <div className="checkout-summary">
        ...
        {pointsToRedeem > 0 && (
          <div className="summary-row discount-row">
            <span>Stars Discount ({pointsToRedeem})</span>
